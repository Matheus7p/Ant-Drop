    ...
    <main>
      <section className="flex flex-col justify-center items-center gap-6">
       <div className="flex flex-col justify-center items-center gap-4">
        ...
        <p className="text-zinc-400 font-medium select-none">Te ajudamos á avaliar a confiabilidade de um produto. Basta colar o link!</p>
       </div>
        ...
        {mutation.isSuccess && mutation.data  &&(
          <div>
            <p className="text-sm font-semibolid text-zinc-600 dark:text-zinc-400"><span className="font-semibold text-lg">Confiabilidade: </span>{calculateProductReliability(mutation.data)}% confiável</p>
          </div>
        )}
      </section>
      ...
      {mutation.isSuccess && mutation.data &&  (
            <Card className="flex flex-col md:flex-row w-auto md:w-[700px] h-auto md:h-[470px] justify-between">
              <CardHeader className="grid-rows-0 gap-2 w-[400]">
                ...
                <p className="font-light text-zinc-600 dark:text-zinc-400"><span className="font-semibold text-lg">Envio: </span>{mutation.data.data.estimatedShipping || "Informação de envio indisponível. Isso pode indicar que o produto não está em estoque ou que eles tercerizam a compra."}</p>
              </CardHeader>
            ...
            <CardHeader>
              ...
              <CardContent>
                <div className="flex flex-col mb-6">
                  ...
                  <strong className="text-zinc-600 dark:text-zinc-400 font-bold text-lg">Reclamações relevantes:</strong> 
                    <ul>
                      {mutation.data.reclameAquiInfo.latestComplaints?.map((complaint, index) => (
                        <div key={index}>
                          <h6 className="text-zinc-600 dark:text-zinc-400 font-bold text-lg">{complaint.title}</h6> 
                          <p className="font-light text-zinc-600 dark:text-zinc-400">{complaint.text}</p>
                        </div>
                      )) || <p className="font-light text-zinc-600 dark:text-zinc-400">Nenhuma reclamação disponível</p>}
                    </ul>
              </CardContent>
          </Card>
        )}
